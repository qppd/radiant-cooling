...
3. Gateway `loop()` drains the queue, computes control values, and writes
   `telemetry/<device>/latest` to Firebase using the **Firebase-ESP-Client**
...
2. The gateway **streams** `radiant/config` in real time (Firebase-ESP-Client
...
- The **gateway** authenticates via Firebase-ESP-Client — **email/password with a
