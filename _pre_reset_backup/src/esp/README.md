...
├── FirebaseSync.{h,cpp}        # cloud:    Firebase RTDB set/update + realtime stream (wraps Firebase-ESP-Client) [gateway only]
...
| `Firebase-ESP-Client` | monitor       | Mobizt; **v4.4.17** (FirebaseESP32 API; setJSON / beginStream / readStream) |
| `WiFiManager`       | monitor         | tzapu - captive-portal WiFi provisioning            |

**ESP32 core 3.x** (by Espressif) is required — `EspNowTransport` and
`FirebaseSync` uses the Firebase-ESP-Client v4.4.17 API.
...
Firebase is **two-way** on the gateway (Mobizt `Firebase-ESP-Client` v4.4.17, async):
